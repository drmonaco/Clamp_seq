...
output$Match = output$Match + xxx_2[,2]
output$`Wild cards` = xxx_2[1,1]
output
 })
```
...
haplotype_str = tmp_t3 %>% group_by(Pair_id) %>% top_n(-1, p_value) %>% top_n(-1, Clamp_name) %>% mutate(haplotype = ifelse(p_value<.001,Nucleotide,"*"))%>% ungroup%>% select(Clamp_name,haplotype,Sample,total,p_value)
